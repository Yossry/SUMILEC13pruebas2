...
class CountryStateCity(models.Model):
	"""
	Model added to manipulate separately the cities on Partnres.partner.doctypeer address.
	"""
	...
	_sql_constraints = [('res_country_state_city_code_uniq', 'unique (code)', 'Ya existe una ciudad con el mismo Nombre')]

	@api.model
	def create(self, vals):

		vals['name'] = str(vals['name']).upper()
		res = super(CountryStateCity, self).create(vals)
	...
	def write(self, vals):
		...
		res= super(CountryStateCity,self).write(vals)
...
class PartnerInfoExtended(models.Model):
	...
	#_sql_constraints = [('ident_unique','UNIQUE(doctype,xidentification)', "Identification number must be unique!")]
	...
	def _concat_name(self):
		...
		if self.x_lastname2:
			self.x_lastname2 = self.self.x_lastname2.upper()
	...
	def _check_ident(self):
		...
		@return: void
		"""
		partner_ids = self.env['res.partner'].search([('xidentification', '=', '')])
		data = []
		if partner_ids:
		...
		for item in self:
			if item.doctype is not "1":
				...
				_logger.info((str(item.xidentification)))
				_logger.info(len(str(item.xidentification)))
				if len(str(item.xidentification)) < 2:
					raise exceptions.ValidationError(msg)
				elif len(str(item.xidentification)) > 14:
					raise exceptions.ValidationError(msg)
